### supplemental growth curve - Tropini + BrkA
...
#read data
tropinibrka <- read_csv('supp_TropiniBrkA/all growth curves - TropiniBrkA.csv')

#calculate technical replicate averages to get biological replicates
tropinibrka_bioreps <- tropinibrka |>
  group_by(time) |>
...
tropinibrka_avg <- tropinibrka_bioreps |>
  group_by(time) |>
...
tropinibrka_sd <- tropinibrka_bioreps |>
  group_by(time) |>
...
tropinibrka_clean <- full_join(tropinibrka_avg, tropinibrka_sd) |>
  select(!(n:conc_sd))

#reorder data by EDTA concentration condition
tropinibrka_clean$conc <- factor(tropinibrka_clean$conc, levels = x_order)

#plot growth curves
growth_curve_tropinibrka <- ggplot(tropinibrka_clean) +
