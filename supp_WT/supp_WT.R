...
#read data
wt <- read_csv('supp_WT/all growth curves - WT.csv')
...
wt_clean <- full_join(wt_avg, wt_sd) |>
  select(!(n:conc_sd))

#reorder data by EDTA concentration condition
wt_clean$conc <- factor(wt_clean$conc , levels = x_order)

#plot growth curves
growth_curve_wt <- ggplot(wt_clean) +
